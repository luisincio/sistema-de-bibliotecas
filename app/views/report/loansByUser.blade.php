...
@section('content')
	<h1>Reporte de préstamos por usuario</h1>

	@if ($errors->has())
		<div class="alert alert-danger" role="alert">
			<p><strong>{{ $errors->first('num_doc') }}</strong></p>
		</div>
	@endif
	...
	<div style="height:140px; position:relative;">
		...
		{{ Form::open(array('url'=>'report/submit_loans_by_user', 'id'=>'toggle-form', 'role'=>'form')) }}
			<div class="col-xs-6" style="margin-bottom:20px;">
				<div class="row">
					<div class="form-group col-xs-8">
						{{ Form::label('num_doc','Número de documento') }}
						{{ Form::text('num_doc',$num_doc,array('class'=>'form-control')) }}
					</div>
				</div>
				{{ Form::submit('Generar reporte',array('id'=>'submit-report', 'class'=>'btn btn-primary')) }}
				<a href="" id="submit_loans_by_user_excel_button" class="btn btn-success"><span class="glyphicon glyphicon-download-alt"></span> Exportar a Excel</a>
	...
	{{ Form::open(array('url'=>'report/submit_loans_by_user_excel', 'id' => 'submit_loans_by_user_excel' ,'role'=>'form')) }}
		{{ Form::hidden('num_doc_excel',$num_doc) }}
